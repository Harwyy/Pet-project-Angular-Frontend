  ...
  export class CanvasComponent implements AfterViewInit {
    ...
    ngAfterViewInit() {
      ...
      context.beginPath();
      context.rect(width * .14, height * 0.5, width * .36, height * .18);
      ...
      context.beginPath();
      context.arc(width * .5, height * .5, width * .18, Math.PI, 3*Math.PI/2, false);
      context.fillStyle = color;
      context.stroke();
      context.fill();
      ...
      context.lineTo(width * .32, height * .5);
      context.lineTo(width * .5, height * .32);
      ...
      canvas.addEventListener('click', (event) => {
        ...
        if (x_number <= -5){
          ...
          this.hitService.updateHit({x: -5, y: y_number, r: 4});
        } else if (x_number == -4) {
          setTimeout(() => {
            // @ts-ignore
            document.getElementById('radio-2').checked = true;
          }, 0);
          this.hitService.updateHit({x: -4, y: y_number, r: 4});
        } else if (x_number == -3){
          setTimeout(() => {
            // @ts-ignore
            document.getElementById('radio-3').checked = true;
          }, 0);
          this.hitService.updateHit({x: -3, y: y_number, r: 4});
        } else if (x_number == -2){
          setTimeout(() => {
            // @ts-ignore
