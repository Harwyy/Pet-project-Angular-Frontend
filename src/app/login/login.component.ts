...
export class LoginComponent {
  ...
  private validateSignUp(): boolean {
    ...
    if (email.length > 5 && password.length > 8) {
      ...
      if (email.length <= 5) {
        // @ts-ignore
        emailErrorElement.textContent = "Email should be longer than 5 characters";
      } else {
        // @ts-ignore
        emailErrorElement.textContent = "";
      }
      if (password.length <= 8) {
        // @ts-ignore
        passwordErrorElement.textContent = "Password should be longer than 8 characters";
      } else {
        // @ts-ignore
        passwordErrorElement.textContent = "";
      }
  ...
  private validateLoginForm() {
    ...
    if (!isValidEmail) {
      // @ts-ignore
      document.getElementById('email-error-login').innerHTML = 'Please enter a valid email address.';
    } else {
      // @ts-ignore
      document.getElementById('email-error-login').innerHTML = '';
    }
